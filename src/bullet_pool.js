...
BulletPool.prototype.update = function(time, callback){
...
BulletPool.prototype.render = function(time, ctx){
  ctx.save();
  ctx.beginPath();
  ctx.fillStyle = "black";
  for(var i = 0; i < this.end; i ++){
    ctx.moveTo(this.pool[4*i], this.pool[4*i+1]);
    ctx.arc(this.pool[4*i], this.pool[4*+1], 2, 0, 2*Math.PI, false);
  }
  ctx.fill();
  ctx.restore();
}
